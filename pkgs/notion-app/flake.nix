{
  description = "Flake for Notion App Electron";
  ...
  in
  {
    packages.x86_64-linux.notionAppElectron = pkgs.stdenv.mkDerivation  rec {
      pname = "notion-app-electron";
      version = "3.14.0";
      src = builtins.fetchGit {
        url = "https://aur.archlinux.org/notion-app-electron.git";
        ref = "master";
        rev = "154a323970a58f4377ca5a1e119e3afe03a5d5a0";
      };
      ...
      installPhase = ''
        ...
        7z x ${notionDmg} "Notion/Notion.app/Contents/Resources/app.asar" -y -bse0 -bso0 || true
        ...
        tar xzf ${betterSqlite} -C asar_patched/node_modules/better-sqlite3/build/Release/
        tar xvf ${bufferUtil} -C asar_patched/node_modules/bufferutil/build/Release/

        # Copy the icon and make patches
        cp ${src}/notion.png asar_patched/.webpack/main/trayIcon.png
        ...
        install -Dm755 ${src}/notion-app $out/bin/notion-app
        install -Dm644 ${src}/notion.desktop $out/share/applications/notion.desktop
        install -Dm644 ${src}/notion.png $out/share/icons/hicolor/256x256/apps/notion.png
      '';
